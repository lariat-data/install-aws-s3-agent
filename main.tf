...
resource "aws_s3_bucket_notification" "lariat_s3_lambda_trigger" {
  for_each = toset(var.target_buckets)
  bucket = each.key
  lambda_function {
    lambda_function_arn = aws_lambda_function.lariat_s3_monitoring_lambda.arn
...
resource "aws_lambda_permission" "s3_lambda_invoke_permission" {
  for_each = toset(var.target_buckets)
  statement_id  = "AllowS3Invoke${index(var.target_buckets, each.key) + 1}"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.lariat_s3_monitoring_lambda.function_name
